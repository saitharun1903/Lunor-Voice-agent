  ...
  const categories = [
    {
      ...
      description: "Keep your existing business phone number. Connect via simple carrier call forwarding or direct SIP trunking.",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {categories.map((cat) => {
            ...
            return (
              <div
                key={cat.title}
                className="rounded-3xl p-8 sm:p-9 backdrop-blur-xl bg-white/80 dark:bg-zinc-900/70 border border-black/[0.08] dark:border-white/[0.12] shadow-lg hover:shadow-2xl hover:border-blue-500/30 transition-all duration-200 flex flex-col justify-between group relative overflow-hidden"
              >
                {/* Top Specular Line */}
                ...
                <div>
                  <div className="flex items-center justify-between mb-6">
                    <div className="w-12 h-12 rounded-2xl bg-blue-500/10 dark:bg-blue-400/10 text-blue-600 dark:text-blue-400 flex items-center justify-center transition-transform duration-200 group-hover:scale-105">
                      <Icon className="w-6 h-6" />
                    </div>
                    <span className="text-xs font-semibold px-3 py-1 rounded-full bg-black/[0.04] dark:bg-white/[0.08] text-zinc-700 dark:text-zinc-300 font-mono">
                      {cat.highlight}
                    </span>
                  </div>
                ...
                <div className="mt-8 pt-4 border-t border-black/[0.04] dark:border-white/[0.06] flex items-center justify-between text-xs text-zinc-500 font-medium font-mono">
